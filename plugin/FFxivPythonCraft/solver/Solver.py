...
from ..Simulator import Manager, Role, Status
from FFxivPythonTrigger.Utils import Logger, Config, GetBall
from FFxivPythonTrigger.Utils.TTS import TTS
from .Utils import *
from .stages import Terminator, Stage1, Stage2, Stage3, Stage4, Stage1_s4
from FFxivPythonTrigger.Utils.i18n import system_to_client_text as _
...
class Solver(object):
    def __init__(self, recall: callable = None, used_config: Config._config = None):
        ...
        if self.use_memFix:
            from FFxivPythonTrigger.Utils import FFxivCraftMem
    ...
    def deal(self, status, prev_skill=None):
        ...
        if ans is not None:
            ...
            if self.recall is not None and self.use_recall: self.recall(ans)
            TTS(ans)
        else:
            print("no recommend")
        return ans
